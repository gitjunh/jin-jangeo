import{p as i,i as h,f as _}from"./assets-DZw7ZkS0.js";async function u(e){const t=i(e),a=await fetch(`${t}?t=${Date.now()}`,{cache:"no-store"});if(!a.ok)throw new Error(`데이터를 불러올 수 없습니다: ${e}`);return a.json()}function v(e,t,a){const n=document.createElement("section");n.className="hero",n.id="top";const l=h()?'<span class="hero__badge hero__badge--closed">오늘 휴무</span>':'<span class="hero__badge hero__badge--open">영업 중</span>';n.innerHTML=`
  ...
  `;const s=n.querySelector(".hero__video");let o=0;const d=t.length>0?t:[{src:"",poster:a}],r=g=>{const p=d[g];s.src=i(p.src),s.poster=i(p.poster),s.play().catch(()=>{})};return r(0),d.length>1&&setInterval(()=>{o=(o+1)%d.length,r(o)},8e3),n}function f(e,t){var n;const a=document.createElement("nav");return a.className="cta-bar",a.setAttribute("aria-label","빠른 메뉴"),a.innerHTML=`
