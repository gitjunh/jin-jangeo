import{f as _}from"./types-bc62W2C5.js";const j="gitjunh",q="jin-jangeo",v="gh-pages",H="3140",E="jin_admin_github_token",g="jin_admin_unlocked";function x(){return sessionStorage.getItem(g)==="1"}function w(){return sessionStorage.getItem(E)}function C(e){sessionStorage.setItem(E,e.trim())}function P(){sessionStorage.setItem(g,"1")}function B(){sessionStorage.removeItem(g)}function U(e){return e===H}function G(){return!!w()}function O(){return!1}function D(){sessionStorage.setItem("jin_local_dev","1")}function R(e){const t=document.createElement("div");t.className="admin-login";let n="";const i=()=>{t.innerHTML=`
    ...
    `;const a=t.querySelector(".admin-login__error");t.querySelectorAll(".pin-pad__key").forEach(s=>{s.addEventListener("click",()=>{const r=s.dataset.key;if(r==="⌫"?n=n.slice(0,-1):n.length<6&&(n+=r),n.length>=4){if(U(n)){P(),a.hidden=!0,e();return}n.length>=6&&(a.textContent="PIN이 올바르지 않습니다.",a.hidden=!1,n="")}i()})})};return i(),t}function J(e){var a,s;const t=document.createElement("div");t.className="admin-setup",t.innerHTML=`
  ...
  `;const n=t.querySelector(".admin-input"),i=t.querySelector(".admin-setup__error");return(a=t.querySelector(".admin-btn"))==null||a.addEventListener("click",()=>{const r=n.value.trim();if(!r.startsWith("github_pat_")&&!r.startsWith("ghp_")){i.textContent="올바른 GitHub 토큰을 입력하세요.",i.hidden=!1;return}C(r),e()}),(s=t.querySelector("[data-local-dev]"))==null||s.addEventListener("click",()=>{D(),e()}),t}function W(e){var n;const t=document.createElement("header");return t.className="admin-header",t.innerHTML=`
    <h1>장어명가 진 관리</h1>
    <button type="button" class="admin-header__logout">나가기</button>
  `,(n=t.querySelector(".admin-header__logout"))==null||n.addEventListener("click",()=>{B(),e()}),t}function F(){return G()?x()?"app":"login":"setup"}const K={maxWidth:1280,imageQuality:.72,videoCrf:30,videoMaxWidth:1280,audioBitrate:"96k"};function z(e,t,n){const i=Math.max(e,t);if(i<=n)return{width:e,height:t};const a=n/i;return{width:Math.round(e*a),height:Math.round(t*a)}}async function Q(e,t=K,n){const i=await createImageBitmap(e),{width:a,height:s}=z(i.width,i.height,t.maxWidth),r=new OffscreenCanvas(a,s),o=r.getContext("2d");if(!o)throw new Error("Canvas 2D context를 사용할 수 없습니다.");return o.drawImage(i,0,0,a,s),i.close(),{blob:await r.convertToBlob({type:"image/webp",quality:t.imageQuality}),width:a,height:s}}function S(e){return`https://api.github.com/repos/${j}/${q}/contents/${e}`}function k(){const e=w();if(!e)throw new Error("GitHub 토큰이 없습니다.");return{Authorization:`Bearer ${e}`,Accept:"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28"}}async function $(e){try{return(await e.json()).message??e.statusText}catch{return e.statusText}}async function L(e){const t=await fetch(`${S(e)}?ref=${v}`,{headers:k()});if(t.status===404)return null;if(!t.ok)throw new Error(await $(t));const n=await t.json();return{sha:n.sha,content:atob(n.content.replace(/\n/g,""))}}async function I(e,t,n){const i=btoa(unescape(encodeURIComponent(JSON.stringify(t,null,2))));await T(e,i,n)}async function Y(e,t,n){const i=await t.arrayBuffer(),a=new Uint8Array(i);let s="";for(let r=0;r<a.length;r++)s+=String.fromCharCode(a[r]);await T(e,btoa(s),n)}async function T(e,t,n){const i={message:`admin: update ${e}`,content:t,branch:v};n&&(i.sha=n);const a=await fetch(S(e),{method:"PUT",headers:{...k(),"Content-Type":"application/json"},body:JSON.stringify(i)});if(!a.ok)throw new Error(await $(a))}async function b(e){const t=await L(e);if(!t)throw new Error(`${e} 파일이 없습니다. 먼저 배포해 주세요.`);return{data:JSON.parse(t.content),sha:t.sha}}async function V(){const{data:e,sha:t}=await b("data/menu.json");return{menu:e,menuSha:t,pendingImages:new Map}}function X(e,t){const n=document.createElement("div");n.className="admin-section";const i=()=>{const s=e.menu.tableSetting;let r=`
  ...
  `}function y(e,t,n){var i;return(i=e.categories.find(a=>a.id===t))==null?void 0:i.items.find(a=>a.id===n)}function ee(e){return e.replace(/"/g,"&quot;")}async function te(e){for(const[,{blob:n,path:i}]of e.pendingImages){const a=await L(i);await Y(i,n,a==null?void 0:a.sha)}await I("data/menu.json",e.menu,e.menuSha);const{sha:t}=await b("data/menu.json");e.menuSha=t,e.pendingImages.clear()}async function ne(){const{data:e,sha:t}=await b("data/restaurant.json");return{info:e,sha:t}}function ae(e,t){const n=document.createElement("div");n.className="admin-section";const i=[{key:"name",label:"식당명"},{key:"tagline",label:"한 줄 소개"},{key:"phone",label:"전화번호"},{key:"mobile",label:"휴대폰"},{key:"address",label:"주소"},{key:"hoursLabel",label:"영업시간"},{key:"closed",label:"휴무일"}];return n.innerHTML=`
    <h2 class="admin-section__title">매장 정보</h2>
    ${i.map(a=>`
      <label class="admin-field">
        <span>${a.label}</span>
        <input class="admin-input" data-key="${a.key}" value="${ie(String(e.info[a.key]??""))}" />
      </label>
    `).join("")}
  `,n.querySelectorAll(".admin-input").forEach(a=>{a.addEventListener("input",()=>{const s=a.dataset.key;e.info[s]=a.value,s==="hoursLabel"&&(e.info.hours=a.value),t()})}),n}function ie(e){return e.replace(/"/g,"&quot;")}async function se(e){await I("data/restaurant.json",e.info,e.sha);const{sha:t}=await b("data/restaurant.json");e.sha=t}async function oe(e){A(e,()=>void M(e))}function A(e,t){const n=F();if(e.innerHTML="",n==="setup"){e.append(J(t));return}if(n==="login"){e.append(R(t));return}t()}async function M(e){e.innerHTML='<p class="loading">불러오는 중…</p>';let t,n,i=!1;try{[t,n]=await Promise.all([V(),ne()])}catch(m){e.innerHTML=`<p class="admin-error">${m instanceof Error?m.message:"로드 실패"}</p>`;return}const a=document.createElement("div");a.className="admin-app";const s=document.createElement("nav");s.className="admin-tabs",s.innerHTML=`
